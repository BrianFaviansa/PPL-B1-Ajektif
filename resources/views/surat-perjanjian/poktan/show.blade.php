...
@section('content')
    <div class="grid grid-cols-2 max-w-5xl gap-x-10 gap-y-6 ml-10">
        ...
        <div>
            <p class="text-xl font-semibold text-gray-900 dark:text-white">Surat SPKO Tingkat 2</p>
        </div>
        <div>
            <p class="text-xl text-gray-900 dark:text-white"><a href="{{ asset('storage/surat_dinas/spkodinas.docx') }}"
                    target="_blank"
        ...
        <div>
            @if ($pengajuan->disetujui_at)
                <p class="text-xl text-gray-900 dark:text-white">{{ $pengajuan->disetujui_at->format('d F Y') }}</p>
            @else
                -
            @endif
        </div>
        <form action="{{ route('perjanjian.unggahSuratPoktan', $pengajuan) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class=>
                <p class="text-xl font-semibold text-gray-900 dark:text-white">Unggah Surat SPKO Poktan</p>
            </div>
            <div class="mt-4 mb-5">
                <input
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-300" id="surat_poktan_help">*Upload surat SPKO Poktan
                </p>
                @error('surat_poktan')
            ...
            <div>
                ...
                <button type="submit"
                    class="max-w-xs focus:outline-none text-white bg-green-700 hover:bg-green-800 focus:ring-4 focus:ring-green-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-green-600 dark:hover:bg-green-700 dark:focus:ring-green-800">Unggah
                    Surat</button>
            </div>
        </form>
    </div>
